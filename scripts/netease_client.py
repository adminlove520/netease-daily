...
from netease_public_api import NeteasePublicAPI, format_song_list as format_public_song_list, push_daily as public_push_daily
            ...
            if songs:
                ...
                for song in songs:
                    ...
                    if song_id in details_map:
                        ...
                        alias = detail.get('alia', [])
                        if alias and not song.get('tags'):
                            song['tags'] = alias[:2]
                print(format_daily_songs(songs))
                return

        # 回退到公开API获取榜单
        print("📡 无登录状态或登录失效，使用公开API获取每日榜单...")
        print(public_push_daily())
